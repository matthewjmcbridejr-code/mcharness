...
async def test_resolve_model_local_first():
    ...
    with patch.object(ProviderGateway, "get_available_ollama_models", return_value=["llama3.2:1b", "llama3.2:3b"]):
        gateway.current_profile = "fast"
        provider, model, profile = await gateway.resolve_model_and_provider()
...
async def test_embedding_model_not_selected_for_chat():
    gateway = ProviderGateway()
    with patch.object(ProviderGateway, "get_available_ollama_models", return_value=["mxbai-embed-large", "llama3.2:1b"]):
        gateway.current_profile = "fast"
        provider, model, profile = await gateway.resolve_model_and_provider()
...
async def test_router_only_model_auto_switch():
    ...
    with patch.object(ProviderGateway, "get_available_ollama_models", return_value=["marius-fast", "llama3.2:1b"]):
