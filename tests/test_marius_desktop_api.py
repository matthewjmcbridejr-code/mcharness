...
def test_get_worker_run_logs_not_found():
    client = TestClient(app)
    response = client.get("/api/marius/worker-runs/non-existent-run-id/logs")
    assert response.status_code == 404
    assert response.json()["detail"] == "Worker run non-existent-run-id not found."
